...
SENTRY_KEY = os.getenv("API_KEY")  # your key from Sentry
...
day_commands = ['monday', 'montag', 'সোমবার', 'tuesday', 'dienstag', 'মঙ্গলবার', 'wednesday', 'mittwoch', 'বুধবা', 'thursday',
                'donnerstag', 'বৃহস্পতিবার', 'friday', 'freitag', 'শুক্রবার', 'saturday', 'samstag', 'শনিবার', 'sunday', 'sonntag', 'রবিবার']
...
def send_hello(message):
    ...
    if str(message.chat.id) not in col:
        ...
        analytics_SUBWAY_BOT.update_acell('F10', str(
            len(database.col_values(1))).replace("'", " "))

    mark_up = InlineKeyboardMarkup()
    B1 = InlineKeyboardButton(text='SUB OF THE DAY!', callback_data='sub')
    B2 = InlineKeyboardButton(
        text='MENU', url='https://www.subway.com/de-DE/MenuNutrition/Menu/Sub-of-the-Day')
    B3 = InlineKeyboardButton(
        text='FEEDBACK', url="https://forms.gle/UnS4hcFamKmDsAKL8")
    ...
    analytics_SUBWAY_BOT.update_acell(
...
def sub(message):
    ...
    subway_bot.send_message(
        message.chat.id, "Check out the *SUB OF THE DAY\! 😋👇🏻*", parse_mode='MarkdownV2')
    sub = sub_of_the_day[day]
    subway_bot.send_photo(message.chat.id, sub)
    sub_count = analytics_SUBWAY_BOT.acell('F4').value
    analytics_SUBWAY_BOT.update_acell(
        'F4', str(int(sub_count)+1).replace("'", " "))
...
(https://www.subway.com/de-DE/MenuNutrition/Menu/Sub-of-the-Day)*", parse_mode='MarkdownV2', disable_web_page_preview=True)
    ...
    analytics_SUBWAY_BOT.update_acell(
        'F5', str(int(menu_count)+1).replace("'", " "))
...
'''
    subway_bot.send_message(
        message.chat.id, contact_info, parse_mode='MarkdownV2')
    contact_count = analytics_SUBWAY_BOT.acell('F6').value
    analytics_SUBWAY_BOT.update_acell(
        'F6', str(int(contact_count)+1).replace("'", " "))
...
def day(message):
    if (message.text).lower() in day_commands:
        ...
        subway_bot.send_message(
            message.chat.id, "Umm...I really don't know how to respond to this message.")
        subway_bot.send_sticker(message.chat.id, umm_stk)

        if message.chat.id != int(my_id):
            subway_bot.send_message(
                my_id, "Hey! Someone just sent me something that I don't understand...")
